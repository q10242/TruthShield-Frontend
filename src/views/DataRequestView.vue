  ...
  <main class="min-h-screen bg-zinc-950 px-6 py-10 text-zinc-100">
    <section class="mx-auto max-w-2xl">
      ...
      <form class="mt-6 space-y-4 rounded-lg border border-white/10 bg-white/[0.03] p-5" @submit.prevent="submit">
        <input v-model="form.email" required type="email" class="w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" :aria-label="t('remaining.emailPlaceholder')" :placeholder="t('remaining.emailPlaceholder')" />
        <select v-model="form.request_type" class="w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" :aria-label="t('remaining.requestType')">
        ...
        <textarea v-model="form.reason" class="min-h-28 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" :aria-label="t('remaining.requestReasonPlaceholder')" :placeholder="t('remaining.requestReasonPlaceholder')"></textarea>
